# maimaiDX 玩家成绩分析与智能练习推荐系统

## 1. 项目简介

maimaiDX 玩家成绩分析与智能练习推荐系统是一个基于 Spring Boot 的后端项目，用于围绕 maimaiDX 玩家成绩数据完成曲库同步、歌曲查询、成绩导入、B50 分析、练习推荐、成长报告和成长趋势对比。
...
推荐接口会基于当前快照成绩、B50 边缘 RA、谱面定数、拟合难度和目标达成率计算候选曲目。系统会生成推荐分数、预计提升值和推荐理由。

推荐逻辑是规则驱动的可解释推荐，不是黑盒模型。生成结果会写入 `recommendation_item` 表。
...
- 规则驱动的可解释练习推荐：推荐结果包含目标达成率、预计提升值和推荐理由。
